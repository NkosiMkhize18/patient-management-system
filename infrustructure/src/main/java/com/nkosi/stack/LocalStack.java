...
public class LocalStack extends Stack {
    ...
    public LocalStack(final App scope, final String id, final StackProps props) {
        ...
        DatabaseInstance authServiceDb =
                createPatientDatabase("AuthServiceDB", "auth-service-db");

        DatabaseInstance patientServiceDb =
                createPatientDatabase("PatientServiceDB", "patient-service-db");

        CfnHealthCheck authDbHealthCheck =
                createDbHealthCheck(authServiceDb, "auth-service-db-health-check");

        CfnHealthCheck patientDbHealthCheck =
                createDbHealthCheck(patientServiceDb, "patient-service-db-health-check");
        ...
        FargateService authService = createFargateService("AuthService", "auth-service",
                List.of(4005),
                authServiceDb,
                Map.of("JWT_SECRET", "+YOkrFuDw0UL7cF0eHEFBduy7T7cZyZ1oEGbSn0rj+w="));
    ...
    private DatabaseInstance createPatientDatabase(String id, String dbName) {
    ...
    private FargateService createFargateService(String id, String imageName, List<Integer> ports, DatabaseInstance db, Map<String, String> additionalVars) {
        ...
        if (db != null) {
            envVars.put("SPRING_DATASOURCE_URL", "jdbc:postgresqsl://%s:%s/%s-db".formatted(
    ...
    public static void main(final String[] args) {
        ...
        //synthesizer is used to covert code into a aws stack template
        StackProps props = StackProps.builder()
                .synthesizer(new BootstraplessSynthesizer()) //only for local dev, skip bootstrapping
                .build();
